    ...
    return (
        <>
            ...
            {isPlaying ? (
                ...
                <TransportBtn onClick={play}><Play size={16} className="text-white ml-0.5" /></TransportBtn>
            )}
...
function TransportBtn({ children, onClick }: { children: ReactNode, onClick?: () => void }) {
    return (
        <button className="w-9 h-9 rounded-md hover:bg-white/[0.08] flex items-center justify-center text-white/60 hover:text-white transition-colors cursor-pointer" onClick={onClick}>
            {children}
        </button>
    );
}