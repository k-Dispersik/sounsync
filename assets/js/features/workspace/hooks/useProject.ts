...
export function useProject(id: number) {
    ...
    const updateClipInState = useCallback((trackId: number, clipId: number, changes: Partial<Clip>) => {
        setProject(prev => {
            if (!prev) return prev;
            return {
                ...prev,
    ...
    const removeTrack = useCallback((trackId: number) => {
        setProject(prev => prev ? { ...prev, tracks: prev.tracks.filter(t => t.id !== trackId) } : prev);
    }, []);

    return { project, isLoading, refetch: fetch, addClip, updateClipInState, updateSettings, addTrack, removeTrack };
}
